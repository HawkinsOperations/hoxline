...
def test_ci_uses_immutable_sibling_revisions_and_all_required_trust_checks() -> None:
    ...
    command_center_ref = "f6d88acac2cbb2ab95482bb4b09dc70bd939a06f"
    assert refs == [command_center_ref]
    assert (
        "HAWKINS_COMMAND_CENTER_IMMUTABLE_OBSERVED_SHA: "
        f"{command_center_ref}"
    ) in workflow
    assert reviewed_manifest in workflow
    assert "Resolve reviewed sibling revisions" in workflow
    for output_name in ("detections", "validation", "platform", "proof", "website"):
        assert f"steps.reviewed.outputs.{output_name}" in workflow
